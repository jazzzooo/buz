    ...
    fn randomUUID(global: *JSGlobalObject, callFrame: *jsc.CallFrame) JSError!JSValue {
        ...
        if (args.len > 0) {
            ...
            if (!options.isUndefined()) {
                const options_object = try validators.validateObject(global, options, "options", .{});
                if (try options_object.get(global, "disableEntropyCache")) |disable_entropy_cache_value| {
...
fn getHashes(global: *JSGlobalObject, _: *jsc.CallFrame) JSError!JSValue {
    ...
    const array = try jsc.JSArray.createEmpty(global, hashes.count());

    for (hashes.keys(), 0..) |hash, i| {
        const str = try String.createUTF8ForJS(global, hash);
        try array.putDirectIndex(global, @intCast(i), str);
    }
