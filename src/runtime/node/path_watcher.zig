...
pub fn watch(
    ...
    const watcher = PathWatcher.new(.{
        .manager = manager,
        .path = bun.handleOom(bun.default_allocator.dupeSentinel(u8, resolved, 0)),
        .recursive = recursive,
        .is_file = is_file,
    });
...
const Linux = struct {
    ...
    fn addOne(
        ...
        for (gop.value_ptr.items) |*o| {
            if (o.watcher == watcher) {
                if (!bun.strings.eql(o.subpath, subpath)) {
                    const old = o.subpath;
                    o.subpath = bun.handleOom(bun.default_allocator.dupeSentinel(u8, subpath, 0));
        ...
        bun.handleOom(gop.value_ptr.append(bun.default_allocator, .{
            .watcher = watcher,
            .subpath = bun.handleOom(bun.default_allocator.dupeSentinel(u8, subpath, 0)),
        }));
