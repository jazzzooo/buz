...
fn findChrome(alloc: std.mem.Allocator, explicitPath: ?[*:0]const u8) !?[:0]const u8 {
    ...
    if (explicitPath) |p| {
        return try alloc.dupeSentinel(u8, std.mem.span(p), 0);
    }
    if (bun.getenvZ("BUN_CHROME_PATH")) |p| return try alloc.dupeSentinel(u8, p, 0);
    ...
    for (names) |n| {
        if (bun.which(buf, path, "", n)) |found| {
            return try alloc.dupeSentinel(u8, found, 0);
        }
    }
    ...
    if (comptime bun.Environment.isMac) {
        ...
        for (absolute) |c| {
            if (bun.sys.isExecutableFilePath(c)) return try alloc.dupeSentinel(u8, c, 0);
        }
    }
...
fn readDevToolsActivePort(out_buf: *std.ArrayListUnmanaged(u8)) ?void {
    ...
    for (candidates) |rel| {
        ...
        var allocating = std.Io.Writer.Allocating.fromArrayList(bun.default_allocator, out_buf);
        defer out_buf.* = allocating.toArrayList();
        allocating.writer.print("ws://127.0.0.1:{d}{s}", .{ port, ws_path }) catch return null;
        return;
    }
    return null;
}
