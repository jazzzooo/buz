fn jsonStringifyDependency(this: *const Lockfile, w: anytype, dep_id: DependencyID, dep: Dependency, res: PackageID) !void {
    ...
    try w.objectField("behavior");
    {
        ...
        const field_names = @typeInfo(Behavior).@"struct".field_names;
        inline for (field_names[1 .. field_names.len - 1]) |field_name| {
            if (@field(dep.behavior, field_name)) {
...
pub fn jsonStringify(this: *const Lockfile, w: anytype) !void {
...

    {
        ...
        for (0..this.packages.len) |i| {
            ...
            if (@as(u16, @backingInt(pkg.meta.arch)) != Npm.Architecture.all_value) {
            ...
            if (@as(u16, @backingInt(pkg.meta.os)) != Npm.OperatingSystem.all_value) {
