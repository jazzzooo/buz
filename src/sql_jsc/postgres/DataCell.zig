...
pub fn fromBytes(binary: bool, bigint: bool, oid: types.Tag, bytes: []const u8, globalObject: *jsc.JSGlobalObject) !SQLDataCell {
    switch (oid) {
        ...
        .int2 => {
            if (binary) {
                return SQLDataCell{ .tag = .int4, .value = .{ .int4 = try parseBinary(.int2, i16, bytes) } };
            } else {
        ...
        .cid, .xid, .oid => {
            if (binary) {
                return SQLDataCell{ .tag = .uint4, .value = .{ .uint4 = try parseBinary(.oid, u32, bytes) } };
            } else {
                return SQLDataCell{ .tag = .uint4, .value = .{ .uint4 = std.fmt.parseInt(u32, bytes, 0) catch 0 } };
            }
        },
        .int4 => {
            if (binary) {
                return SQLDataCell{ .tag = .int4, .value = .{ .int4 = try parseBinary(.int4, i32, bytes) } };
            } else {
                return SQLDataCell{ .tag = .int4, .value = .{ .int4 = std.fmt.parseInt(i32, bytes, 0) catch 0 } };
            }
        },
        ...
        .bool => {
            if (binary) {
                return SQLDataCell{ .tag = .bool, .value = .{ .bool = @intFromBool(bytes.len > 0 and bytes[0] == 1) } };
            } else {
        ...
        .date, .timestamp, .timestamptz => |tag| {
            ...
            if (binary and bytes.len == 8) {
                switch (tag) {
                    .timestamptz => return SQLDataCell{ .tag = .date_with_time_zone, .value = .{ .date_with_time_zone = types.date.fromBinary(bytes) } },
                    .timestamp => return SQLDataCell{ .tag = .date, .value = .{ .date = types.date.fromBinary(bytes) } },
                    else => unreachable,
                }
            } else {
...
pub const Putter = struct {
    ...
    fn putImpl(this: *Putter, index: u32, optional_bytes: ?*Data, comptime is_raw: bool) !bool {
        ...
        const cell: *SQLDataCell = &this.list[index];
        if (is_raw) {
