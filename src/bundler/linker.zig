...
pub const Linker = struct {
    ...
    pub fn getModKey(
        ...
        const file: std.Io.File = if (fd) |_fd| _fd.stdFile() else try std.Io.Dir.cwd().openFile(this.resolver.io, file_path.text, .{ .mode = .read_only });
        Fs.FileSystem.setMaxFd(file.handle);
        const modkey = try Fs.FileSystem.RealFS.ModKey.generate(&this.fs.fs, this.resolver.io, file_path.text, file);

        if (fd == null)
            file.close(this.resolver.io);
        return modkey;
    }
