...
pub fn enable() void {
    ...
    if (comptime Environment.isLinux) {
        ...
        _ = std.posix.prctl(.SET_PDEATHSIG, .{@backingInt(std.posix.SIG.KILL)}) catch return;
...
fn parentPidOf(pid: std.c.pid_t) std.c.pid_t {
    ...
    if (comptime Environment.isLinux) {
        ...
        const path = std.mem.printSentinel(&path_buf, "/proc/{d}/stat", .{pid}, 0) catch return 0;
...
fn listChildPidsLinux(parent: std.c.pid_t, out: []std.c.pid_t) ?usize {
    ...
    while (it.next().unwrap() catch null) |entry| {
        ...
        const children_path = std.mem.printSentinel(&path_buf, "/proc/{d}/task/{d}/children", .{ parent, tid }, 0) catch continue;
