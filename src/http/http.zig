...
fn getTlsHostname(client: *const HTTPClient, allowProxyUrl: bool) []const u8 {
    ...
    if (client.tls_props) |props| {
        if (props.get().server_name) |sn| {
            const sn_slice = std.mem.sliceTo(sn, 0);
                ...
                if (client.signals.get(.cert_errors)) {
                    ...
                    client.state.certificate_info = .{
                        ...
                        .cert_error = .{
                            ...
                            .code = bun.handleOom(bun.default_allocator.dupeSentinel(u8, certError.code, 0)),
                            .reason = bun.handleOom(bun.default_allocator.dupeSentinel(u8, certError.reason, 0)),
                        },
                    };
        ...
        if (!ssl_ptr.isInitFinished()) {
            ...
            if (!strings.isIPAddress(_hostname)) {
                if (_hostname.len < temp_hostname.len) {
                    ...
                    hostname = bun.default_allocator.dupeSentinel(u8, _hostname, 0) catch unreachable;
            ...
            switch (status_code) {
                302, 301, 307, 308, 303 => {
...

                    {
                        ...
                        var fba: std.heap.BufferFirstAllocator = .init(&fba_buffer, url_arena.allocator());
                        const url_allocator = fba.allocator();
