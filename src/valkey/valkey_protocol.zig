...
pub const RESPType = enum(u8) {
    ...
    pub fn fromByte(byte: u8) ?RESPType {
        return switch (byte) {
            @backingInt(RESPType.SimpleString) => .SimpleString,
            @backingInt(RESPType.Error) => .Error,
            @backingInt(RESPType.Integer) => .Integer,
            @backingInt(RESPType.BulkString) => .BulkString,
            @backingInt(RESPType.Array) => .Array,
            @backingInt(RESPType.Null) => .Null,
            @backingInt(RESPType.Double) => .Double,
            @backingInt(RESPType.Boolean) => .Boolean,
            @backingInt(RESPType.BlobError) => .BlobError,
            @backingInt(RESPType.VerbatimString) => .VerbatimString,
            @backingInt(RESPType.Map) => .Map,
            @backingInt(RESPType.Set) => .Set,
            @backingInt(RESPType.Attribute) => .Attribute,
            @backingInt(RESPType.Push) => .Push,
            @backingInt(RESPType.BigNumber) => .BigNumber,
            else => null,
        };
    }
};
