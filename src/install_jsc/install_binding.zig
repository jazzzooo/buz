pub const bun_install_js_bindings = struct {
    ...
    pub fn generate(global: *JSGlobalObject) JSValue {
        ...
        obj.putDirect(global, parseLockfile, jsc.JSFunction.create(global, "parseLockfile", jsParseLockfile, 1, .{}).toJS());
        return obj.toJS();
    }
    ...
    pub fn jsParseLockfile(globalObject: *JSGlobalObject, callFrame: *jsc.CallFrame) bun.JSError!JSValue {
        ...
        const manager = globalObject.bunVM().transpiler.resolver.ensurePackageManager() catch |err| {
            return globalObject.throwError(err, "failed to initialize package manager");
        };
