...
`JSCallback` has experimental support for callbacks invoked by arbitrary native threads. Enable it with the optional `threadsafe` parameter. Bun queues each invocation on the JavaScript context that created the callback, so thread-safe callbacks are asynchronous and must return `void`.

Thread-safe callback arguments may be scalar or pointer FFI types. JavaScript-managed argument types such as `buffer`, `napi_env`, and `napi_value` are not supported. Any native memory referenced by a pointer argument must remain valid until the callback runs.

```ts
const onResult = new JSCallback(
  (ptr, length) => {
    console.log(new CString(ptr, length).toString());
  },
  {
    returns: "void",
    args: ["ptr", "usize"],
